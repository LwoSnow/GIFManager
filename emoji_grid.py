...
class EmojiGridWidget(QWidget):
    ...
    def clear_layout_cache(self):
        """Clear layout caches so the next layout pass recalculates sizes."""
        ...
        for ci, lst in cols.items():
            nw = max(
                (EmojiItem.text_natural_width(e.get("text_content", ""))
                 if e.get("text_content") else EmojiItem.get_card_size())
                for e in lst
            )
    ...
    def _rearrange(self):
        ...
        spacing = self._flow.spacing()
        # Column-width baseline: max natural width in the group (card_size for images / text natural
        # width), so every column is fully displayed
        # 列宽基准：组内最大自然宽（图片 card_size / 文字自然宽），保证每列完整显示
        base_w = EmojiItem.get_card_size()
        for em in self._data:
            if em.get("text_content"):
    ...
    def _blank_zone_col(self, exclude=None):
        ...
        nw = (EmojiItem.text_natural_width(dragged.get("text_content", ""))
              if dragged.get("text_content") else EmojiItem.get_card_size())
        usable = self._usable_width()
        spacing = self._flow.spacing()
        ...
        for em in self._data:
            ...
            w = (EmojiItem.text_natural_width(em.get("text_content", ""))
                 if em.get("text_content") else EmojiItem.get_card_size())
    ...
    def _merge_overflow_columns(self):
        ...
        for em in self._data:
            ...
            if em.get("text_content"):
                ...
                nw = EmojiItem.get_card_size()  # Image cards use dynamic size / 图片卡片动态宽
