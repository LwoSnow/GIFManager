...
from PySide6.QtGui import (
    QDragEnterEvent, QDropEvent, QFont, QIcon, QAction, QKeyEvent, QKeySequence,
    QMouseEvent,
)
...
class MainWindow(QMainWindow):
    ...
    def _load_settings(self):
        ...
        # Emoji card size / 表情卡片大小
        self._emoji_size = self._settings.value("emoji_size", 100, type=int)
        self._emoji_size = max(60, min(200, self._emoji_size))
        EmojiItem.set_global_size(self._emoji_size)
        # Name font size / 名字字号
        self._name_font_size = self._settings.value("name_font_size", 10, type=int)
        self._name_font_size = max(1, min(20, self._name_font_size))
        EmojiItem.set_name_font_size(self._name_font_size)
    ...
    def _save_settings(self):
        ...
        self._settings.setValue("emoji_size", self._emoji_size)
        self._settings.setValue("name_font_size", self._name_font_size)
    ...
    def _open_settings(self):
        ...
        dlg = SettingsDialog(
            ...
            name_font_size=self._name_font_size,
    ...
    def _apply_settings(self, dlg):
        ...
        # Emoji size / 表情大小
        new_size = dlg.emoji_size()
        if new_size != self._emoji_size:
            self._emoji_size = new_size
            EmojiItem.set_global_size(new_size)
            # Resize existing cards
            for card in self.emoji_grid._items:
                if not card._is_text:
                    card.setFixedSize(EmojiItem.get_card_size(), EmojiItem.card_height(card._show_name))
                    card._width = EmojiItem.get_card_size()
                    if hasattr(card, '_thumb_label') and card._thumb_label is not None:
                        ts = EmojiItem.get_thumb_size()
                        card._thumb_label.setFixedSize(ts, ts)
            self.emoji_grid.clear_layout_cache()
            self.emoji_grid._flow.relayout()
            self.emoji_grid._flow.activate()
            self.emoji_grid.update()
        # Name font size / 名字字号
        new_font_size = dlg.name_font_size()
        if new_font_size != self._name_font_size:
            self._name_font_size = new_font_size
            EmojiItem.set_name_font_size(new_font_size)
            for card in self.emoji_grid._items:
                if not card._is_text and card._show_name and hasattr(card, '_name_label'):
                    _f = QFont()
                    _f.setPointSize(max(1, new_font_size))
                    card._name_label.setFont(_f)
                    card._name_label.setFixedHeight(EmojiItem.get_name_area())
                    card.setFixedSize(EmojiItem.get_card_size(), EmojiItem.card_height(card._show_name))
            self.emoji_grid.clear_layout_cache()
            self.emoji_grid._flow.relayout()
            self.emoji_grid._flow.activate()
